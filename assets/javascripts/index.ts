...
(function () {
  ...
  document.addEventListener('click', function (e) {
    if ((<HTMLTextAreaElement>e.target) && (<HTMLTextAreaElement>e.target).className === 'list-group-item user') {
      const users = document.getElementsByClassName('user');
      const userId: string = (<HTMLTextAreaElement>e.target).getAttribute('data-id');
    ...
    return new Promise(function (resolve, reject) {
      ...
      xhr.onload = function () {
        if (this.status === 200) {
          ...
          let error: any = new Error(this.statusText);
